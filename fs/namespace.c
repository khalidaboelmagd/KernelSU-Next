...
static void mnt_free_id(struct mount *mnt)
{
	int id = mnt->mnt_id;
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	/* Fake mnt_ids handed out by susfs_alloc_unshare_ksu_vfsmnt() never
	 * came from mnt_id_ida, so they must not be released back into it.
	 */
...
static int mnt_alloc_group_id(struct mount *mnt)
{
	...
	bool ksu_range = false;

#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	/* Mounts created by the ksu domain take their peer group id from a
	 * high reserved range so they stay distinguishable from ordinary peer
	 * groups. Upstream susfs uses ida_alloc_min(); this tree predates that
	 * API, so the equivalent is ida_get_new_above() with a raised floor.
	 */
	...
	res = ida_get_new_above(&mnt_group_ida,
				start,
				&mnt->mnt_group_id);
	/* Only advance the shared cursor for ordinary allocations, so a ksu
	 * mount landing at DEFAULT_KSU_MNT_GROUP_ID cannot drag every later
	 * ordinary peer group up into the reserved range.
	 */
	if (!res && !ksu_range)
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
/* A copy of alloc_vfsmnt() but reuses the caller-supplied mnt_id for mounts
 * that are unshared by a ksu process. The id does not come from mnt_id_ida,
 * so VFSMOUNT_MNT_FLAGS_KSU_UNSHARED_MNT is set by the caller and mnt_free_id()
 * skips releasing it.
 */
...
/* A copy of alloc_vfsmnt() but allocates mnt_id from the reserved high range
 * for a mount that is mounted or single-cloned by a ksu process.
 * Upstream susfs uses ida_alloc_min(); this tree predates that API, so the
 * equivalent is ida_get_new_above() with DEFAULT_KSU_MNT_ID as the floor.
 */
